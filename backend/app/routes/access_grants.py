...
from app.rbac import is_global_admin, normalized_role, require_permission, scope_permission_query
...
def list_grants(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if not is_global_admin(current_user):
        query = scope_permission_query(query, db, current_user, "access.connect", AccessGrant.server_id)
...
def revoke(grant_id: int, payload: schemas.RevokeIn, request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    grant = db.get(AccessGrant, grant_id)
    if not grant:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Grant not found")
    require_permission(db, current_user, "access.revoke", server_id=grant.server_id, conceal=True)
    require_step_up(db, current_user, "manual_revoke", request, reason="Manual revoke requires MFA step-up")
...
def import_logs(grant_id: int, request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    grant = db.get(AccessGrant, grant_id)
    if not grant:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "Grant not found")
    require_permission(db, current_user, "commands.view_group", server_id=grant.server_id, conceal=True)
