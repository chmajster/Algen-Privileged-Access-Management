...
async def test_web_connection(db: DBSession, resource: dict[str, Any], connection: dict[str, Any], inputs: dict[str, SecretInput]) -> list[CheckResult]:
    ...
    try:
        ...
        guard = NavigationGuard(allowed, bool(connection.get("allow_private_network", False)), connection.get("blocked_domains", [])); _, addresses = await guard.validate(url)
        ...
        context = await _guarded_web_context(url, allowed, connection.get("blocked_domains", []), bool(connection.get("allow_private_network", False)))
