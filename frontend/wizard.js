(() => {
  ...
  const safeName = (environment, name) => {
    const part = (value) => String(value ?? "").replace(/[Łł]/g, "L").normalize("NFKD").replace(/[\u0300-\u036f]/g, "").toUpperCase().replace(/[^A-Z0-9]+/g, "-").replace(/^-|-$/g, "");
  ...
  const wizard = {
    ...
    authStep() {
      ...
      if (this.resourceType === "ssh") {
        const auth = this.field("connection.authentication_type", "private_key");
        const rotation_auth = this.field("connection.rotation_authentication_type", "password");
        return `<div class="form-grid wizard-fields"><div class="span-2"><h4 class="mb-3">Konto pośrednie (Połączeniowe)</h4></div>${this.select("connection.authentication_type","Metoda uwierzytelniania",[["private_key","Klucz prywatny"],["password","Hasło"],["agent","Agent SSH"]])}${auth === "private_key" ? this.secretPicker("connection.secret_ref_id","connection.secret_input_key","ssh_auth","Klucz prywatny SSH") : auth === "password" ? this.secretPicker("connection.secret_ref_id","connection.secret_input_key","ssh_auth","Hasło SSH") : ""}<div class="span-2 mt-4"><h4 class="mb-3">Konto rotacyjne / weryfikacyjne</h4></div>${this.select("connection.rotation_authentication_type","Metoda uwierzytelniania",[["password","Hasło (Zapisane w sejfie)"],["private_key","Klucz prywatny"],["one_time","Jednorazowe (podawane ręcznie przy wykonaniu)"],["agent","Agent SSH"]])}${rotation_auth === "private_key" ? this.secretPicker("connection.rotation_secret_ref_id","connection.rotation_secret_input_key","rotation_auth","Klucz prywatny SSH") : rotation_auth === "password" ? this.secretPicker("connection.rotation_secret_ref_id","connection.rotation_secret_input_key","rotation_auth","Hasło SSH") : rotation_auth === "one_time" ? '<div class="alert alert-info span-2 mb-0">Hasło jednorazowe nie będzie przechowywane w bazie. System poprosi o nie tuż przed rotacją.</div>' : ""}</div>`;
      }
    ...
    permissionsStep() {
      ...
      return `<div class="form-grid wizard-fields"><label class="form-label span-2">Użyj istniejącego profilu dostępu<select class="form-select" data-bind="access_group_id"><option value="">Utwórz nowy safe</option>${groups.map((g)=>`<option value="${g.id}" ${String(this.field("access_group_id"))===String(g.id)?"selected":""}>${esc(g.name)}</option>`).join("")}</select><small>Safe określa dozwolony typ dostępu i limity.</small></label>${!this.field("access_group_id")?`${this.input("access_profile.name","NAZWA safe'a","Operatorzy ERP — standard","Wpisana wartość utworzy końcowy segment NAZWA.")}<div class="field-note span-2"><div>Finalna nazwa: <strong data-safe-name-preview>${esc(generatedName || "SRODOWISKO-ENV-CUSTOM-NAZWA")}</strong></div><small>ŚRODOWISKO pochodzi z wyboru zasobu, ENV-CUSTOM jest stałe, a NAZWA z pola powyżej.</small></div>${this.input("access_profile.description","Opis profilu","Dostęp operacyjny bez pełnego sudo","", "textarea")}${this.select("access_profile.access_option","Poziom uprawnień",[["ssh_only","Zwykły dostęp"],["limited_sudo","Ograniczone sudo"],["full_sudo","Pełne sudo"]])}`:""}</div>`;
    },
    ...
    deriveDomain(value) { try { const url=new URL(value.includes("://")?value:`https://${value}`);if(!this.data._domain_manually_edited){this.set("connection.allowed_domains",[url.hostname]);this.set("connection.allowed_domains_text",url.hostname);const el=this.root.querySelector('[data-bind="connection.allowed_domains_text"]');if(el)el.value=url.hostname;}if(!this.data._name_manually_edited){this.set("resource.name",url.hostname.split(".")[0]);const el=this.root.querySelector('[data-bind="resource.name"]');if(el)el.value=url.hostname.split(".")[0];}}catch(_){} },
